...
const submit = async (req,res) => {
  try {
    const task = await getTask(req.params.id);
    if (!task) return res.status(404).json({success:false,message:'Tarea no encontrada'});
    if (['Finalizado','Cancelado'].includes(task.proyecto_estado)) return res.status(400).json({success:false,message:'El proyecto no permite nuevas entregas'});
    const { comentario_aprendiz, url_entrega, ruta_archivo } = req.body;
    if (!comentario_aprendiz && !url_entrega && !ruta_archivo) return res.status(400).json({success:false,message:'Debes aportar comentario, enlace o archivo'});
    const [existing] = await db.query('SELECT * FROM entregas_tareas WHERE id_tarea=?',[req.params.id]);
    if (existing.length) {
      if (req.user.rol !== 'Administrador' && existing[0].id_aprendiz !== Number(req.user.id)) return res.status(403).json({success:false,message:'La entrega pertenece a otro aprendiz'});
      if (req.user.rol !== 'Administrador' && existing[0].estado !== 'Requiere corrección') return res.status(400).json({success:false,message:'La entrega no está habilitada para corrección'});
    ...
    if (req.user.rol !== 'Administrador' && Number(req.user.id) !== idAprendiz) return res.status(403).json({success:false,message:'Solo el aprendiz asignado puede entregar esta tarea'});
