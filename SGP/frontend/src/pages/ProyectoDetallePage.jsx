...
// NUEVO: límite de palabras/caracteres para el "apartado de mensajes".
// Se valida aquí (mismo límite en las 3 vistas — Administrador, Instructor
// y Aprendiz comparten esta misma pestaña) y también en el backend
// (ver backend/src/validators/common.validators.js → limitedText), para
// que nadie pueda saltárselo llamando la API directamente.
const MENSAJE_MAX_CARACTERES = 1500;
const MENSAJE_MAX_PALABRAS   = 300;
const COMENTARIO_MAX_CARACTERES = 2000;
const COMENTARIO_MAX_PALABRAS   = 300;

function contarPalabras(texto) {
  return texto.trim() ? texto.trim().split(/\s+/).filter(Boolean).length : 0;
}

// Contador reutilizable "123/1500 caracteres · 12/300 palabras" que cambia
// a rojo cuando se supera el límite.
function ContadorTexto({ texto, maxCaracteres, maxPalabras }) {
  const caracteres  = texto.length;
  const palabras    = contarPalabras(texto);
  const excedido    = caracteres > maxCaracteres || palabras > maxPalabras;
  return (
    <div style={{ fontSize:11, marginTop:4, textAlign:'right', color: excedido ? 'var(--red-600)' : 'var(--slate-500)' }}>
      {caracteres}/{maxCaracteres} caracteres · {palabras}/{maxPalabras} palabras
    </div>
  );
}
...
export default function ProyectoDetallePage() {
  ...
  const [msgError,  setMsgError]  = useState(''); // NUEVO
  const [enviandoMsg, setEnviandoMsg] = useState(false); // NUEVO
  ...
  // ── Enviar mensaje ────────────────────────────────
  // NUEVO: valida el límite de palabras/caracteres en el cliente antes de
  // llamar a la API (el backend valida lo mismo, ver mensajes.routes.js,
  // por si alguien intenta saltarse esta pantalla).
  async function enviarMensaje(e) {
    e.preventDefault();
    setMsgError('');
    const texto = msgTexto.trim();
    if (!texto) return;
    if (texto.length > MENSAJE_MAX_CARACTERES) {
      setMsgError(`El mensaje no puede superar los ${MENSAJE_MAX_CARACTERES} caracteres`);
      return;
    }
    if (contarPalabras(texto) > MENSAJE_MAX_PALABRAS) {
      setMsgError(`El mensaje no puede superar las ${MENSAJE_MAX_PALABRAS} palabras`);
      return;
    }
    setEnviandoMsg(true);
    try {
      await mensajesService.create({ contenido: texto, id_proyecto: id });
      setMsgTexto('');
      const r = await mensajesService.getByProyecto(id);
      setMensajes(r.data.data || []);
    } catch (err) {
      // 429 = límite anti-spam del backend (máx. 10 mensajes/minuto)
  ...
  // Mismo criterio de límite de palabras/caracteres que en enviarMensaje().
  async function agregarComentario(e) {
    e.preventDefault();
    setDetalleError('');
    const texto = nuevoComentario.trim();
    if (!texto) return;
    if (texto.length > COMENTARIO_MAX_CARACTERES) {
      setDetalleError(`El comentario no puede superar los ${COMENTARIO_MAX_CARACTERES} caracteres`);
      return;
    }
    if (contarPalabras(texto) > COMENTARIO_MAX_PALABRAS) {
      setDetalleError(`El comentario no puede superar las ${COMENTARIO_MAX_PALABRAS} palabras`);
      return;
    }
    try {
      await comentariosService.create({ contenido: texto, id_entregable: entregableSel.id_entregable });
      setNuevoComentario('');
            ...
            <form onSubmit={enviarMensaje}>
              <div style={{ display:'flex', gap:10, alignItems:'flex-start' }}>
                <textarea
                  ...
                  value={msgTexto}
                  onChange={e => { setMsgTexto(e.target.value); if (msgError) setMsgError(''); }}
                ...
                <button
                  ...
                  className="btn btn-primary"
                  disabled={
                    !msgTexto.trim() || enviandoMsg ||
                    msgTexto.length > MENSAJE_MAX_CARACTERES ||
                    contarPalabras(msgTexto) > MENSAJE_MAX_PALABRAS
                  }
                >
                  {enviandoMsg ? 'Enviando…' : 'Enviar ✉️'}
                </button>
              </div>
              <ContadorTexto texto={msgTexto} maxCaracteres={MENSAJE_MAX_CARACTERES} maxPalabras={MENSAJE_MAX_PALABRAS} />
            </form>
          </div>
        )}
        ...
        <div className="modal-overlay" onClick={() => setModalDetalle(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()} style={{ maxWidth:560 }}>
            ...
            <div className="modal-body" style={{ maxHeight:'70vh', overflowY:'auto' }}>
              ...
              <form onSubmit={agregarComentario} style={{ marginBottom:20 }}>
                <div style={{ display:'flex', gap:8 }}>
                  <textarea className="form-textarea" style={{ flex:1, resize:'vertical' }} rows={2}
                    placeholder="Escribe un comentario…" maxLength={COMENTARIO_MAX_CARACTERES}
                    value={nuevoComentario} onChange={e=>setNuevoComentario(e.target.value)} />
                  <button type="submit" className="btn btn-primary btn-sm"
                    disabled={
                      !nuevoComentario.trim() ||
                      nuevoComentario.length > COMENTARIO_MAX_CARACTERES ||
                      contarPalabras(nuevoComentario) > COMENTARIO_MAX_PALABRAS
                    }>
                    Enviar
                  </button>
                </div>
                <ContadorTexto texto={nuevoComentario} maxCaracteres={COMENTARIO_MAX_CARACTERES} maxPalabras={COMENTARIO_MAX_PALABRAS} />
              </form>
